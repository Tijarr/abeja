...
export default function SpaceEditForm({ slug, name, description, color, domainName, taskCount, contactCount, documentCount }: {
  slug: string; name: string; description: string; color: string; domainName: string
  taskCount: number; contactCount: number; documentCount: number
}) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setMessage('Guardado')
        if (data.slugChanged) {
          router.push(`/space/${formSlug}/edit`)
        } else {
          router.refresh()
        }
      }
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <div className="flex items-center gap-3 ml-6 text-[11px]" style={{ color: 'var(--text-tertiary)' }}>
          <span>{domainName}</span>
          <span>·</span>
          <span>{taskCount} tarea{taskCount !== 1 ? 's' : ''}</span>
          {contactCount > 0 && <><span>·</span><span>{contactCount} contacto{contactCount !== 1 ? 's' : ''}</span></>}
          {documentCount > 0 && <><span>·</span><span>{documentCount} doc{documentCount !== 1 ? 's' : ''}</span></>}
        </div>
      </div>
      ...
      <div className="mb-8">
        ...
        <textarea
          ...
          rows={8}
          placeholder="Describe este espacio. Este texto sirve como contexto para ti y para agentes IA..."
